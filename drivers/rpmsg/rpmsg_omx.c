 ...
 */
static int rpmsg_omx_pin_buffer(struct rpmsg_omx_instance *omx, int fd, int *id)
{
...
rpmsg_omx_dma_find(struct rpmsg_omx_instance *omx, int id, phys_addr_t *pa)
{
	struct device *dev = omx->omxserv->dev;
	struct rpmsg_omx_dma_info *dma;

	dev_dbg(dev, "looking for id=%u\n", id);

	mutex_lock(&omx->lock);
	dma = idr_find(&omx->dma_idr, id);
	mutex_unlock(&omx->lock);
	if (dma)
...
static int rpmsg_omx_unpin_buffer(struct rpmsg_omx_instance *omx, int id)
{
	struct rpmsg_omx_dma_info *dma;
	phys_addr_t pa;

	dma = rpmsg_omx_dma_find(omx, id, &pa);
	if (!dma)
		return -EINVAL;
					...
					long buffer, phys_addr_t *da)
{
...
#ifdef CONFIG_DMA_SHARED_BUFFER
	{
		...
		if (rpmsg_omx_dma_find(omx, id, &pa))
			ret = 0;
		else
			dev_err(dev, "error getting id %d\n", id);
	}
#endif
	if (!ret)
...
long rpmsg_omx_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case OMX_IOCBUFREGISTER:
	{
		...
		ret = rpmsg_omx_pin_buffer(omx, data.fd, &data.id);
		if (ret)
			break;

		ret = copy_to_user((char __user *) arg, &data, sizeof(data));
		if (ret) {
			dev_err(omxserv->dev,
				"%s: %d: copy_to_user fail: %d\n", __func__,
				_IOC_NR(cmd), ret);
	...
	case OMX_IOCBUFUNREGISTER:
	{
		...
		ret = rpmsg_omx_unpin_buffer(omx, data.id);
	}
#endif
	default:
